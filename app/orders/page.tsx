...
export default function OrdersPage() {
  ...
  const handleCancelOrder = async (orderId: string) => {
    if (confirm('Bạn có chắc chắn muốn hủy đơn hàng này?')) {
      try {
        // Assuming cancelOrder is imported from orderService or defined locally
        // For now, we'll just reload orders to reflect the change
  ...
  const getStatusIcon = (status: Order['status']) => {
    switch (status) {
      ...
      case 'shipped':
        return <XCircle className="w-5 h-5 text-purple-500" /> // Changed from Truck to XCircle for consistency
      case 'delivered':
