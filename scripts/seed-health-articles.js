...
const firebaseConfig = {
  ...
  authDomain: "hoanglinhmedicine.firebaseapp.com",
  projectId: "hoanglinhmedicine",
  storageBucket: "hoanglinhmedicine.appspot.com",
...
async function seedHealthArticles() {
  try {
    console.log('Bắt đầu thêm bài viết sức khỏe...')
    
    for (const article of healthArticles) {
      await addDoc(collection(db, 'health-articles'), article)
      console.log(`Đã thêm bài viết: ${article.title}`)
    }
    
    console.log('Hoàn thành thêm bài viết sức khỏe!')
  } catch (error) {
